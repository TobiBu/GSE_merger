...
p = plt.hist(data_main['v_phi'],bins=50, histtype='step', label='$\mathrm{before\, merger}$', density=True)
#print(np.mean(data_main['v_phi_gas']))
data_main2 = pickle.load(open(paths.data / '2.79e12..00480_main_halo_total_age_fe.dat','rb'))
p = plt.hist(data_main2['v_phi'],bins=50, histtype='stepfilled', label='$\mathrm{after\, merger}$', density=True)
#print(np.mean(data_main2['v_phi_gas']))
plt.legend()
plt.xlabel('$v_\\phi\, [\mathrm{km/s}]$')
plt.ylabel('$\mathrm{d} f/\mathrm{d} v_\\phi$')
plt.savefig(paths.figures / '2.79e12_v_phi_stars.pdf', bbox_inches='tight')