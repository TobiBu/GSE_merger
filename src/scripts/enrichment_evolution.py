...
for i, f in enumerate(gas_profile_files[::-1]):
    ...
    cen = np.where((0 < data['bins']) & (data['bins'] < 3))
    mid2 = np.where((3 < data['bins']) & (data['bins'] < 6))
    mid = np.where((7 < data['bins']) & (data['bins'] < 10))
    mid3 = np.where((10 < data['bins']) & (data['bins'] < 14))
    out = np.where((14 < data['bins']) & (data['bins'] < 17))
